...
// Uses the default Wire instance: SDA=9, SCL=40 (defined in pins_arduino.h)

// --- SPI Bus (main, for FRAM and Display) -----------------------------------
// MOSI=11, MISO=10, SCK=13 (defined in pins_arduino.h)
// Do NOT redefine PIN_SPI_* here — they come from the variant header.

// --- LoRa SX1262 (dedicated internal SPI bus — no user config needed) -------
// Has its own SPI on pins 3/5/6/7 plus control lines.
// These are defined in pins_arduino.h as LORA_SX126X_* and used by RadioLib.

// --- UART1 (Slot D — GNSS) -------------------------------------------------
// Per pins_arduino.h: TX=43, RX=44
#define PIN_GNSS_TX             PIN_SERIAL1_TX  // GPIO 43
#define PIN_GNSS_RX             PIN_SERIAL1_RX  // GPIO 44
...
#define PIN_FRAM_CS             WB_A1   // GPIO 2
...
// Both display types use IO Slot 1 (SPI-based)
    ...
    // RAK14000 E-Ink Display (3.52", 360x240, tri-colour)
    #define PIN_DISP_CS         WB_IO3  // GPIO 41
    #define PIN_DISP_DC         WB_IO5  // GPIO 38
    #define PIN_DISP_RST        WB_IO6  // GPIO 39
    #define PIN_DISP_BUSY       WB_IO4  // GPIO 42
    #define DISPLAY_TYPE_NAME   "E-Ink (RAK14000)"
    #define DISPLAY_WIDTH       360
    #define DISPLAY_HEIGHT      240

#elif defined(DISPLAY_TFT) && DISPLAY_TFT == 1
    // RAK14014 TFT LCD Display (2.4", 240x320, IPS, touch)
    #define PIN_DISP_CS         WB_IO3  // GPIO 41
    #define PIN_DISP_DC         WB_IO5  // GPIO 38
    #define PIN_DISP_RST        WB_IO6  // GPIO 39
    #define PIN_DISP_BL         WB_IO4  // GPIO 42 — backlight control
...
struct __attribute__((packed)) SensorRecord {
    ...
    int16_t  temperature;       // degrees C x 100    (2345 = 23.45 C)
    uint16_t humidity;          // percent x 10       (655  = 65.5%)
    uint16_t pressure;          // hPa x 10           (10132 = 1013.2 hPa)
    uint32_t gasResistance;     // Ohms raw
    uint16_t pm1_0;             // ug/m3
    uint16_t pm2_5;             // ug/m3
    uint16_t pm10;              // ug/m3
    uint16_t lux;               // lux x 10           (5432 = 543.2 lux)
...
struct __attribute__((packed)) LoRaPayload {
    int16_t  temperature;       // degrees C x 100
    uint16_t humidity;          // percent x 10
    uint16_t pressure;          // hPa x 10
    uint32_t gasResistance;     // Ohms
    uint16_t pm2_5;             // ug/m3
    uint16_t pm10;              // ug/m3
    uint16_t lux;               // lux x 10
