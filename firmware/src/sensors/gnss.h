...
public:
    ...
    // --- Position and satellite data ---

    int32_t latitudeScaled() {
        if (!hasValidFix()) return SENSOR_UNAVAILABLE_I32;
        return static_cast<int32_t>(_gps.location.lat() * 1e7);
    }

    int32_t longitudeScaled() {
        if (!hasValidFix()) return SENSOR_UNAVAILABLE_I32;
        return static_cast<int32_t>(_gps.location.lng() * 1e7);
    }

    double latitude()      { return _gps.location.lat(); }
    double longitude()     { return _gps.location.lng(); }
    double altitude()      { return _gps.altitude.meters(); }

    // --- Satellite info ---

    uint32_t satellites()  { return _gps.satellites.value(); }
    double hdop()          { return _gps.hdop.hdop(); }
    ...
    uint16_t year()        { return _gps.date.year(); }
    uint8_t month()        { return _gps.date.month(); }
    uint8_t day()          { return _gps.date.day(); }
    uint8_t hour()         { return _gps.time.hour(); }
    uint8_t minute()       { return _gps.time.minute(); }
    uint8_t second()       { return _gps.time.second(); }
    ...
    void getStatusString(char* buf, size_t len) {
        if (!_available) {
